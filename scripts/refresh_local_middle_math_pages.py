...
def refresh_page(html: str, slug: str) -> str:
    ...
    if not article:
        return html
    individualized = individualize_local_middle_math_body(article.group(2), slug)
    ...
    search_title, meta_description = build_local_middle_math_meta(slug, individualized)
...
def main() -> int:
    ...
    for path in sorted(OUTPUT.glob("*/index.html")):
        ...
        updated.append(slug)
    print(f"checked local middle-math pages: {checked}")
    print(f"updated local middle-math pages: {len(updated)}")
