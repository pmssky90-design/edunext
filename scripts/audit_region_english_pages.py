...
def main() -> int:
    ...
    for path in OUTPUT.glob("*/index.html"):
        ...
        if not slug.endswith("영어과외") or slug == "영어과외":
            continue
        html = path.read_text(encoding="utf-8")
        if SUBJECT_MARKER not in html:
            continue
        ...
        location = slug.removesuffix("영어과외")
        bad = [phrase for phrase in BAD_PHRASES if phrase in article_text]
        pages[slug] = {
