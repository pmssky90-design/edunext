...
from sitegen.local_high_english import SCHOOL_CONTEXT, build_local_high_english_meta


SLUG_PATTERN = re.compile(r"^(부산|양산|구미).+(?:동|읍|면)고등영어과외$")
...
def _focus(soup: BeautifulSoup) -> str:
    marker = soup.select_one("article [data-high-english-focus]")
    return str(marker.get("data-high-english-focus") or "").strip() if marker else ""
...
def _visible_faq(soup: BeautifulSoup) -> list[tuple[str, str]]:
    heading = soup.select_one("article h2.high-english-faq[data-faq-focus]")
    if not heading:
        return []
...
def audit_errors(pages: dict[str, dict[str, object]]) -> list[str]:
    ...
    required_sections = (
        "high-english-diagnosis",
        "high-english-search-intent",
        "high-english-grade",
        "high-english-school-context",
        "high-english-practice",
        "high-english-schedule",
        "high-english-student-case",
        "high-english-decision",
        "high-english-focus-language",
        "high-english-deep-dive",
        "high-english-error-map",
        "high-english-focus-protocol",
    )
    for slug, page in pages.items():
        ...
        markers = article.select('[data-content-version="high-english-individual-v4"]')
        if len(markers) != 1:
            errors.append(f"{slug}: content marker count {len(markers)}")
        for forbidden in ("중학생", "중학교", "중1", "중2", "중3", "middle-english"):
            if forbidden in str(article):
                errors.append(f"{slug}: middle-school residue {forbidden}")
        for token in ("saan", "Bish", "Li?", "TODO", "FIXME"):
            if token in str(article):
                errors.append(f"{slug}: malformed token {token}")
        if int(page["chars"]) < 9000:
            errors.append(f"{slug}: short individualized content {page['chars']}")
        ...
        for section_class in required_sections:
            count = len(article.select(f".{section_class}"))
            if count != 1:
                errors.append(f"{slug}: section {section_class} count {count}")
        ...
        for paragraph in article.select("p"):
            ...
            if location not in text and focus not in text:
                errors.append(f"{slug}: paragraph lacks local focus: {text[:60]}")

        school_section = article.select_one("section.high-english-school-context")
        if school_section:
            expected_links = [
                school["homepage"] for school in SCHOOL_CONTEXT.get((city, town), [])[:4] if school["homepage"]
            ]
            links = school_section.select("a.source-link[href]")
        ...
        search_section = article.select_one("section.high-english-search-intent")
        if search_section:
            if len(search_section.select("table")) != 1 or len(search_section.select("tbody tr")) != 3:
                errors.append(f"{slug}: search intent table")
            if len(search_section.select("ol")) != 1 or len(search_section.select("ol > li")) != 4:
                errors.append(f"{slug}: search intent steps")
            if search_section.select("a[href]"):
                errors.append(f"{slug}: link inside search intent")

        case_section = article.select_one("section.high-english-student-case")
        if case_section:
            text = case_section.get_text(" ", strip=True)
            if case_section.get("data-case-model") != "composite" or "합성" not in text and "가상" not in text:
                errors.append(f"{slug}: composite case disclosure")
            if case_section.get("data-case-grade") not in {"고1", "고2", "고3"}:
                errors.append(f"{slug}: case grade")
            if len(case_section.select("tbody tr")) != 3 or len(case_section.select("ol > li")) != 3:
                errors.append(f"{slug}: case structure")
            if case_section.select("a[href]"):
                errors.append(f"{slug}: link inside case")

        context_section = article.select_one("aside.high-english-context-links")
        if context_section:
            links = context_section.select("a[href]")
            expected = [f"/{city}고등영어과외/", f"/{location}영어과외/"]
            hrefs = [str(link.get("href") or "") for link in links]
            if context_section.get("data-link-count") != "2" or hrefs != expected:
        ...
        allowed_links = article.select("section.high-english-school-context a.source-link[href], aside.high-english-context-links a[href]")
        if len(article.select("a[href]")) != len(allowed_links):
            errors.append(f"{slug}: unexpected article link")

        visible, schema = _visible_faq(soup), _schema_faq(soup)
        if len(visible) != 5:
        ...
        faq_heading = article.select_one("h2.high-english-faq")
        if faq_heading:
        ...
        expected_title, expected_description = build_local_high_english_meta(slug, str(article))
