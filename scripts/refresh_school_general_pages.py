...
from sitegen.school_general import (
    build_school_general_body,
    build_school_general_meta,
    is_school_general_slug,
    school_general_contexts,
)
...
def refresh_page(html: str, slug: str, source_body: str) -> str:
    ...
    if not article:
        return html
    body = build_school_general_body(slug, source_body)
    body = re.sub(r'(<h[23]\b[^>]*?)\s+id=["\'][^"\']+["\']', r"\1", body, flags=re.I)
    search_title, meta_description = build_school_general_meta(slug, source_body)
...
def main() -> int:
    ...
    expected = set(school_general_contexts())
    updated: list[str] = []
    missing: list[str] = []
    ...
    extra = [
        ...
        if is_school_general_slug(path.parent.name) and path.parent.name not in expected
    ]
    print(f"expected school general pages: {len(expected)}")
    print(f"updated school general pages: {len(updated)}")
    print(f"missing school general pages: {len(missing)}")
    print(f"missing source bodies: {len(missing_source)}")
    print(f"unexpected school general pages: {len(extra)}")
