...
from sitegen.local_elementary_english import (
    build_local_elementary_english_meta,
    individualize_local_elementary_english_body,
    is_local_elementary_english_slug,
)
...
def refresh_page(html: str, slug: str) -> str:
    ...
    if not article:
        return html
    individualized = individualize_local_elementary_english_body(article.group(2), slug)
    ...
    search_title, meta_description = build_local_elementary_english_meta(slug, individualized)
...
def main() -> int:
    ...
    for path in sorted(OUTPUT.glob("*/index.html")):
        ...
        updated.append(slug)
    print(f"checked local elementary-English pages: {checked}")
    print(f"updated local elementary-English pages: {len(updated)}")
