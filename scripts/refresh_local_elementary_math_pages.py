...
def refresh_page(html: str, slug: str) -> str:
    ...
    if not article:
        return html
    individualized = individualize_local_elementary_math_body(article.group(2), slug)
    ...
    search_title, meta_description = build_local_elementary_math_meta(slug, individualized)
...
def main() -> int:
    ...
    for path in sorted(OUTPUT.glob("*/index.html")):
        ...
        updated.append(slug)
    print(f"checked local elementary-math pages: {checked}")
    print(f"updated local elementary-math pages: {len(updated)}")
