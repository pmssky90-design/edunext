...
def _visible_faq(article: BeautifulSoup) -> list[tuple[str, str]]:
    heading = article.select_one("h2.school-english-faq")
    if not heading:
        return []
...
def load_pages(root: Path) -> dict[str, dict[str, object]]:
    ...
    for slug in sorted(school_english_contexts()):
...
def audit_errors(pages: dict[str, dict[str, object]]) -> list[str]:
    ...
    contexts = school_english_contexts()
    for slug, context in contexts.items():
        ...
        if len(article.select('[data-content-version="school-english-individual-v1"]')) != 1:
            errors.append(f"{slug}: content marker")
        if int(page["chars"]) < 8000:
            errors.append(f"{slug}: short content {page['chars']}")
        if int(page["tables"]) < 4 or int(page["items"]) < 6:
            errors.append(f"{slug}: weak structured content")
        if len(page["headings"]) < 18 or len(page["paragraphs"]) < 38:
            errors.append(f"{slug}: weak section density")
        for token in ("속도을", "TODO", "FIXME", "실제 학생 A", "점수를 보장"):
        ...
        if len(soup.select("main h1")) != 1 or soup.select_one("main h1").get_text(" ", strip=True) != slug:
        ...
        expected_internal = [f"/{context.general_slug}/", f"/{context.math_slug}/", f"/{context.region_english_slug}/"]
        ...
        title, description = build_school_english_meta(slug, str(article))
        if page["title"] != title or not (20 <= len(title) <= 60):
            errors.append(f"{slug}: title")
        if page["description"] != description or not (90 <= len(description) <= 160):
...
def main() -> int:
    ...
    average, maximum, _ = similarity_summary(current)
    if maximum > 0.40:
