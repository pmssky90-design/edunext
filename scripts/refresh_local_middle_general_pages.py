...
def _middle_general_faq_schema(body: str) -> dict[str, object] | None:
    section = re.search(
        r'<section\b[^>]*class="[^"]*\bmiddle-general-faq\b[^"]*"[^>]*>(.*?)</section>',
        body,
        flags=re.I | re.S,
    )
    if not section:
...
def _replace_schema(html: str, body: str, search_title: str, meta_description: str) -> str:
    ...
    faq = _middle_general_faq_schema(body)
    if faq:
        data.append(faq)
...
def refresh_page(html: str, slug: str) -> str:
    ...
    if not article:
        return html
    individualized = individualize_local_middle_general_body(article.group(2), slug)
    individualized = re.sub(r'(<h[23]\b[^>]*?)\s+id=["\'][^"\']+["\']', r"\1", individualized, flags=re.I)
    search_title, meta_description = build_local_middle_general_meta(slug, individualized)
...
def main() -> int:
    ...
    for path in sorted(OUTPUT.glob("*/index.html")):
        ...
        updated.append(slug)
    print(f"checked local middle-general pages: {checked}")
    print(f"updated local middle-general pages: {len(updated)}")
