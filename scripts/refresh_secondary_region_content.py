...
from sitegen.render import add_contextual_region_links, deduplicate_region_body_links, enhance_content_body, replace_regional_faq
from sitegen.secondary_region_content import SECONDARY_REGION_SLUGS, individualize_secondary_region_body
from sitegen.utils import region_meta_description
from scripts.refresh_priority_region_content import replace_hero_description
...
def main() -> int:
    ...
    missing = sorted(SECONDARY_REGION_SLUGS - page_map.keys())
    if missing:
        print(f"missing secondary pages: {', '.join(missing)}")
        return 1

    changed = 0
    for slug in sorted(SECONDARY_REGION_SLUGS):
        page = page_map[slug]
        ...
        if not article:
            ...
            return 1
        body = individualize_secondary_region_body(article.group(1), page)
    ...
    print(f"refreshed secondary region content: {changed}")
    return 0
