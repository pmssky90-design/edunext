...
from sitegen.school_math import (
    build_school_math_body,
    build_school_math_meta,
    is_school_math_slug,
    school_math_contexts,
)
...
def refresh_page(html: str, slug: str, source_body: str) -> str:
    article = ARTICLE_PATTERN.search(html)
    if not article:
        return html
    body = build_school_math_body(slug, source_body)
    body = re.sub(r'(<h[23]\b[^>]*?)\s+id=["\'][^"\']+["\']', r"\1", body, flags=re.I)
    search_title, meta_description = build_school_math_meta(slug, source_body)
...
def main() -> int:
    ...
    expected = set(school_math_contexts())
    updated: list[str] = []
    missing: list[str] = []
    ...
    for slug in sorted(expected):
        ...
        refreshed = refresh_page(html, slug, source_body)
        if refreshed != html:
    ...
    extra = [
        ...
        if is_school_math_slug(path.parent.name) and path.parent.name not in expected
    ]
    print(f"expected school math pages: {len(expected)}")
    print(f"updated school math pages: {len(updated)}")
    print(f"missing school math pages: {len(missing)}")
    print(f"missing source bodies: {len(missing_source)}")
    print(f"unexpected school math pages: {len(extra)}")
    return 0 if len(expected) == 143 and not missing and not missing_source and not extra else 1
