...
from sitegen.local_elementary_english import CONTENT_VERSION, build_local_elementary_english_meta
from sitegen.local_elementary_math import ELEMENTARY_SCHOOL_CONTEXT


SLUG_PATTERN = re.compile(r"^(부산|양산|구미).+(?:동|읍|면)초등영어과외$")
...
def _focus(soup: BeautifulSoup) -> str:
    marker = soup.select_one("article [data-elementary-english-focus]")
    return str(marker.get("data-elementary-english-focus") or "").strip() if marker else ""
...
def _visible_faq(soup: BeautifulSoup) -> list[tuple[str, str]]:
    heading = soup.select_one("article h2.elementary-english-faq[data-faq-focus]")
    if not heading:
        return []
...
def audit_errors(pages: dict[str, dict[str, object]]) -> list[str]:
    ...
    for slug, page in pages.items():
        ...
        if len(page["paragraphs"]) < 30 or len(page["headings"]) < 30:
            errors.append(f"{slug}: thin structure")
        if int(page["tables"]) != 3:
            errors.append(f"{slug}: table count {page['tables']}")

        search = article.select("section.elementary-english-search-intent")
        if len(search) != 1:
            errors.append(f"{slug}: search section count {len(search)}")
        else:
            if location not in search[0].get_text(" ", strip=True) or focus not in search[0].get_text(" ", strip=True):
                errors.append(f"{slug}: local focus missing from search section")
            ...
            if search[0].select("a[href]"):
                errors.append(f"{slug}: link inside search section")

        expected_sections = {
            "section.elementary-english-grade": 3,
            "section.elementary-english-four-skills": 4,
            "section.elementary-english-error-map": 5,
            "section.elementary-english-focus-protocol": 6,
        }
        for selector, h3_count in expected_sections.items():
            sections = article.select(selector)
            if len(sections) != 1 or len(sections[0].select("h3")) != h3_count:
                errors.append(f"{slug}: bad section {selector}")
        protocol = article.select_one("section.elementary-english-focus-protocol")
        if not protocol or protocol.get("data-protocol-cards") != "6":
            errors.append(f"{slug}: protocol card marker")

        school = article.select_one("section.elementary-english-school-context")
        if not school:
        ...
        context = article.select("aside.elementary-english-context-links")
        if len(context) != 1:
            ...
            expected = [f"/{city}초등영어과외/", f"/{location}영어과외/"]
        ...
        expected_links = 2 + sum(
            1 for school_item in ELEMENTARY_SCHOOL_CONTEXT.get(location, [])[:4] if school_item.get("homepage")
        )
        if int(page["links"]) != expected_links:
            errors.append(f"{slug}: unexpected article links {page['links']} != {expected_links}")
        ...
        expected_title, expected_description = build_local_elementary_english_meta(slug, str(article))
