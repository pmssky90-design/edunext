...
from sitegen.local_high_general import (
    build_local_high_general_meta,
    individualize_local_high_general_body,
    is_local_high_general_slug,
)
from sitegen.render import enhance_content_body, plain_text
...
SCHEMA_PATTERN = re.compile(r'(<script\s+type="application/ld\+json">)(.*?)(</script>)', flags=re.I | re.S)


def _high_general_faq_schema(body: str) -> dict[str, object] | None:
    section = re.search(
        r'<section\b[^>]*class="[^"]*\bhigh-general-faq\b[^"]*"[^>]*>(.*?)</section>',
        body,
        flags=re.I | re.S,
    )
    if not section:
    ...
    entities = [
        ...
        if plain_text(question) and plain_text(answer)
    ]
...
def _replace_schema(html: str, body: str, search_title: str, meta_description: str) -> str:
    ...
    faq = _high_general_faq_schema(body)
    if faq:
        data.append(faq)
...
def _replace_search_meta(html: str, search_title: str, meta_description: str) -> str:
    ...
    replacements = (
        ...
        (
            r'(<section\s+class="page-hero">\s*<p\s+class="eyebrow">.*?</p>\s*<h1>.*?</h1>\s*<p>).*?(</p>)',
...
def refresh_page(html: str, slug: str) -> str:
    ...
    if not article:
        return html
    individualized = individualize_local_high_general_body(article.group(2), slug)
    individualized = re.sub(r'(<h[23]\b[^>]*?)\s+id=["\'][^"\']+["\']', r"\1", individualized, flags=re.I)
    search_title, meta_description = build_local_high_general_meta(slug, individualized)
...
def main() -> int:
    ...
    for path in sorted(OUTPUT.glob("*/index.html")):
        ...
        updated.append(slug)
    print(f"checked local high-general pages: {checked}")
    print(f"updated local high-general pages: {len(updated)}")
