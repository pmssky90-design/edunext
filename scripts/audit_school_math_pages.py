...
from sitegen.school_math import build_school_math_meta, school_math_contexts, school_math_focus
...
def _visible_faq(article: BeautifulSoup) -> list[tuple[str, str]]:
    heading = article.select_one("h2.school-math-faq")
    if not heading:
        return []
...
def load_pages(root: Path) -> dict[str, dict[str, object]]:
    ...
    for slug in sorted(school_math_contexts()):
...
def audit_errors(pages: dict[str, dict[str, object]]) -> list[str]:
    ...
    for slug, context in contexts.items():
        ...
        focus = school_math_focus(slug, source_body)
        if len(article.select('[data-content-version="school-math-individual-v1"]')) != 1:
            errors.append(f"{slug}: content marker")
        if int(page["chars"]) < 9000:
            errors.append(f"{slug}: short content {page['chars']}")
        if int(page["tables"]) < 5 or int(page["items"]) < 9:
        ...
        expected_internal = [f"/{context.general_slug}/", f"/{context.english_slug}/", f"/{context.region_math_slug}/"]
        ...
        for question, answer in visible:
            ...
            if slug not in question or "수학" not in question or not question.endswith("?"):
                errors.append(f"{slug}: FAQ question focus")
            if len(answer) < 150:
                errors.append(f"{slug}: FAQ answer too short {len(answer)}")

        title, description = build_school_math_meta(slug, source_body)
