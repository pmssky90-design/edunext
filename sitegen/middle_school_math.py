...
METHODS = (
    ("간격 복원", "수업 뒤에는 핵심 개념과 첫 식을 회상하고 간격을 둔 다음 조건을 바꾼 문제에 적용합니다", "첫 회상·후속 적용·독립 재현에서 달라진 부분을 한 줄씩 이어 적습니다", "설명이 끊긴 단계만 다시 학습해 복습 간격을 조정합니다"),
    ...
    ("단계별 풀이 복원", "첫 시도에서 오류 위치를 표시하고 간격을 둔 뒤 첫 줄부터 다시 풀며 다음에는 변형 문제에 적용합니다", "도움을 받은 줄과 혼자 이어 간 줄을 다른 기호로 남깁니다", "간격을 둔 시도에서도 같은 곳에서 멈추면 선행 개념으로 돌아갑니다"),
...
ACTION_LIBRARY = (
    ...
    ("주기 조정", "유지·변경·중단 행동을 하나씩 선택", "도움 없이 수행한 단계 수", "다음 학습 목표 한 가지 합의", "과제 부담이 크면 범위보다 크기부터 축소"),
)
...
def _weekly(context: MiddleSchoolMathContext, theme: tuple[str, ...], method: tuple[str, ...]) -> str:
    ...
    table = _table(
        ...
        (
            ...
            ("수업 뒤 첫 복원", method[1], method[2]),
            ("주말 짧은 점검", "대표 오답 한 문제 빈 풀이", method[3]),
        ),
    )
    paragraphs = (
        ...
        f"한 번의 긴 학습보다 수업 직후와 간격을 둔 뒤의 짧은 재현이 무엇을 잊었는지 보여 줍니다. {escape(context.slug)} 주간 계획에는 시작 시각, 실제 집중 분량, 도움 없이 끝낸 단계만 기록합니다.",
        f"과제를 못 끝낸 날은 양을 그대로 다음 날로 밀지 않습니다. 읽기·첫 식·계산·검산 중 멈춘 단계를 표시하고, 다음 시작 과제를 짧게 끝낼 수 있는 크기로 줄여 다시 연결합니다.",
    )
...
def _action_cycle(context: MiddleSchoolMathContext, theme: tuple[str, ...], method: tuple[str, ...]) -> str:
    ...
    intro = (
        ...
        f'<h2>{escape(context.display_name)} 학교자료 기반 순환형 실행 순서</h2>'
...
def _scenario(context: MiddleSchoolMathContext, theme: tuple[str, ...], method: tuple[str, ...]) -> str:
    table = _table(
        ("가상 시점", "관찰된 행동", "조정 예시"),
        (
            ("출발 관찰", theme[1], theme[2]),
            ("설명 뒤 적용", "해설 직후에는 풀지만 빈 종이에서 첫 줄이 멈춤", method[1]),
            ("원인 비교", "틀린 위치를 계산 실수라고만 표시", "최초 오류 코드와 다음 확인 신호 기록"),
            ("독립 재현", "도움 없이 설명한 단계가 일부 늘어남", "유지할 행동과 새 목표를 분리"),
        ),
    )
    paragraphs = (
...
def _tutor_choice(context: MiddleSchoolMathContext, theme: tuple[str, ...]) -> str:
    table = _table(
        ...
        (
            ("초기 학습 기간에 무엇을 진단하나요?", f"시험지와 {theme[2]}를 함께 확인", "점수만 보고 진도부터 확정"),
...
def _feedback(context: MiddleSchoolMathContext, method: tuple[str, ...]) -> str:
    table = _table(
        ("점검 단계", "학생 기록", "보호자 확인", "다음 결정"),
        (
            ("출발 확인", "시작 가능한 과제 크기", "귀가·수면 흐름", "시간대 조정"),
            ("원인 확인", "오류 코드와 질문 한 줄", "완료 여부보다 막힌 단계", "설명 방식 조정"),
            ("간격 재현", "첫 회상과 후속 재현", "도움 요청 시점", "복습 간격 조정"),
            ("독립 점검", "혼자 설명한 단계", "부담·지속 가능성", "유지·변경·중단"),
        ),
    )
    paragraphs = (
